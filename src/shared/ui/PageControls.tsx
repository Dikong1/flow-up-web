            ...
            >
                {LANGUAGES.map(({ code, label }) => (
                    <DropdownMenuItem
                        ...
                        className="flex cursor-pointer items-center gap-1 rounded-lg px-3 py-1.5"
                    >
                        <span className="font-mono text-xs font-medium tracking-widest uppercase">
                            {code}
                        </span>
                        <span className="text-muted-foreground flex-1 text-right text-[12px]">
                            {label}
                        </span>
                    </DropdownMenuItem>
                ))}
...
export const PageControls = () => {
    return (
        <div className="border-border/50 bg-background/70 fixed right-6 bottom-6 z-50 flex items-center gap-1 rounded-full border px-2 py-1.5 shadow-lg backdrop-blur-xl">
            <ThemeToggle />
