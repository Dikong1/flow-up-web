...
const aggregateCumulativeByWeek = (
...
const CustomTooltip = ({ active, payload, label }: CustomTooltipProps) => {
    ...
    const remaining =
        payload.find((p) => p.dataKey === 'remaining')?.value ?? 0;
    const all = completed + remaining;

    return (
        <div className="bg-foreground rounded-md border p-2 text-white shadow dark:text-black">
            <p className="font-medium">{label}</p>
            <p>
                {t('statistics.all')}: {all}
            </p>
            <p>
                {t('statistics.completed')}: {completed}
            </p>
            <p>
                {t('statistics.remaining')}: {remaining}
            </p>
        </div>
    );
};
